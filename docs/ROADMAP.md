...
**Phases 19–21 (README Enhancement & Wiki Reconciliation) planned (2026-08-19).** Full scope: the wiki page `Plan-Phases-19-21-README-Wiki-Enhancement`. Documentation-only chapter: optimize `README.md` into an accurate, current, wiki-wired front door (fix the §2 gaps: no wiki links, missing Makefile surface, no current-state summary, `copykit-smoke` absent from System Flow, stale status lines), then reconcile the wiki with the repo `docs/` tree (including the false Agent-Guide "no `docs/` directory" claim) and record the canonical-docs-home decision. **P19** (repo PR, gates run) — README baseline audit + optimization + native verification; **P20** (wiki pushes, no gates) — Agent-Guide fix + docs-home decision, wiki↔docs sync, plan page push; **P21** — cross-surface consistency matrix + close-out truthing (one `delivery-telemetry` dispatch to confirm DORA metrics unchanged). Status: **planned** — milestones #17–#19 and planning issues #148–#155 created; no phase work executed yet.
