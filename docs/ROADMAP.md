...
- **Known gaps**: none open in the engine (Phases 0–21 complete). Phase 7 added delivery telemetry & audit trail atop GitHub's native records; Phases 8–11 delivered the maiden voyage, product surface, release-train/failure telemetry, and the release-train simulator; Phases 12–14 operated train 2 and validated the copy-kit; Phases 15–18 reconciled the branch topology (ADR 0011), flew train 2 with documented slip decisions (release `v0.3.0`), proved the copy-kit via the in-repo consumer-path rehearsal (`copykit-smoke.yml`), and closed the chapter with the final truthing readout (P18); Phases 19–21 made the README an accurate, wiki-wired front door and reconciled the wiki with `docs/` (ADR 0012, docs-home decision). **Phases 19–21 complete 2026-08-19** — see the wiki page `Plan-Phases-19-21-README-Wiki-Enhancement`.
...
**Phase 17 (Adoption Proof: Copy-Kit) executed via the plan's documented in-repo substitution.** Full scope: `docs/plans/phases-15-18-cadence-continuity-adoption.md`; evidence: `docs/evidence/p17-consumer-repo-flight.md`. The scratch consumer repo `frdrpo/pdm-copykit-consumer` proved the engine natively in a foreign repo (PR #1 — **10/10 checks green**, 3 PR gates + Risk/Security/compliance/publish-pages, merged `1cf46d9`) before vanishing from the account with no trail. The phase then completed per the plan's flagged fallback: `copykit-smoke.yml` (dispatch) + `scripts/consumer-smoke.mjs` rehearse the kit §1→§8 path in a throwaway consumer workspace on a fresh runner (`make test-consumer-path` locally); `make topology-check`/`topology-apply` automate kit §2 (`scripts/wire-topology.mjs`). Every GitHub-side friction from the real flight is folded into kit §1/§2/§8 (Pages requires a public repo on the free plan; `required_reviewers` PUT on the environment, not `protection_rules`; the visibility-change guardrail; the parity-branch PR guard; dispatch-registration 404 timing). **Phase 16 (Train 2) executed 2026-08-18 via documented slip decisions** (see Phase 16 paragraph); **Phase 18 close-out in PR #145**.

**Phase 18 (Chapter Close-out & ROADMAP Readout) executed (2026-08-18).** Full scope: `docs/plans/phases-15-18-cadence-continuity-adoption.md`. **P18-T1** — this close-out PR #145: ROADMAP §13 status updated through Phase 18; ADR 0011 indexed (ADR log); chapter plan marked complete; planning issues closed; `make lint` green; `main` parity verified (main promoted to `245a9afa` + this PR's close-out payload). **P18-T2** — final truthing snapshot (run 32115287697, 90d window — see Phase 16 paragraph for the readout) recorded in ROADMAP + `docs/release-train.md`; calendar advanced with trains 3/4 windows listed; title-hygiene re-scan: **zero** non-drill classifier matches in Phases 15–18 work. One coherent delivered chapter.

**Phases 19–21 (README Enhancement & Wiki Reconciliation) complete (2026-08-19).** Full scope: the wiki page `Plan-Phases-19-21-README-Wiki-Enhancement`. Documentation-only chapter: `README.md` is now an accurate, current, wiki-wired front door (Current State section with Phases 0–18/`v0.3.0`/P16-T4 readout, 8/8 Makefile targets, `copykit-smoke` in System Flow, wiki Documentation section); this file's status lines synced to Phases 0–18; **ADR 0012** records the docs-home model (wiki canonical, `docs/` committed mirror); the false Agent-Guide "no `docs/` directory" claim fixed on both surfaces; wiki↔`docs/` reconciled with zero residual content drift. **P19** executed via PRs #156/#157 (all gates green natively); **P20** via wiki pushes (`b6ee25f`, `6136709`); **P21-T1** consistency matrix showed zero unresolved diffs; **P21-T2** close-out `delivery-telemetry` (run 32205889156, 90d window) confirmed DORA metrics unchanged (CFR 1.0% 1/100, 100 deployments, 3 releases, on-time 100% — identical to baseline; tiny DF/LT/MTTR proxy deltas are 90d-window-edge + snapshot-time effects from this read-only run, not pollution). Planning issues #148–#155 and milestones #17–#19 closed.
