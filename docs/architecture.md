...
- `release-on-tag.yml` triggers on `v*` tags: builds the deployable, generates release notes (commits since the previous tag, or the most recent commits when no previous tag exists; the AI-assisted draft summary from `scripts/release-summary.mjs` is appended as review/edit guidance), creates a GitHub Release, and uploads the release notes as an artifact.
