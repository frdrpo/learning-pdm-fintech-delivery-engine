...
Status: **opening (2026-08-18)**. This chapter makes the engine's two remaining promises real: operating at cadence, not just in bursts, and adoption by a different product.
...
> All statements verified 2026-08-18 against the working tree, git history, `make lint`, and the live GitHub API — unless marked `needs-verification`.
...
- **P15-T1** — Record the topology decision as **ADR 0011** (restore `develop` as the integration+default branch vs. codify single-trunk `main`). Recommendation by weight of repo canon (every workflow, the runbook, the copy-kit, and ADR 0002/0005's "dispatch registers on the default branch" all assume the two-branch model): **restore `develop`** as default + integration branch and re-create it at current `main` parity — but the decision body evaluates both options with the verified evidence (current live state, workflow references above). *Gates:* `quality-gate`, `risk-health-check`, `compliance-guardrail`. *Touchpoints:* `docs/decisions/`.
- **P15-T2** — Repair the release chain per ADR 0011: re-create `develop` at `main` parity (if restoring) *or* re-plumb `release-on-tag`'s bump target to `main` with protection-aware handling (if single-trunk); re-verify the milestone-gated `workflow_dispatch` path is self-contained and does not rely on deleted branches. Verify with a dispatch run that still auto-creates/opens the `v0.3.0` milestone and blocks as designed (gate behavior test, no release created). *Gates:* `quality-gate` (actionlint), `release-pipeline` (dispatch), `delivery-telemetry` (no accidental new failure events). *Touchpoints:* `.github/pdm/workflows/` (+ `make sync`), repo-settings, default-branch setting.
- **P15-T3** — Repair `publish-pages`: trigger on the reconciled base branch (restore `push: [develop]` or switch to `main`), confirm the `github-pages` env branch policy matches (exact allow-list verified and recorded), and confirm a push re-publishes Pages so `DEPLOY_VERIFY_URL` verify stays live; manual `workflow_dispatch` re-check as fallback. *Gates:* `quality-gate`, `release-pipeline`. *Touchpoints:* `.github/pdm/workflows/`, environment settings (`needs-verification` on the branch allow-list resolution).
- **P15-T4** — Realign `.github/dependabot.yml` `target-branch` and any repo-settings references to the reconciled topology; confirm dependabot opens against the right branch (one real dependabot PR observed — e.g. bump — or `needs-verification` recorded otherwise). *Touchpoints:* `.github/dependabot.yml`, repo-settings.
- **P15-T5** — Reconcile the docs: ROADMAP §13 status line, `docs/architecture.md` (workflow map triggers + promotion chain), `docs/local-runbook.md` (dispatch `--ref develop` guidance), `docs/agents-guide.md`, `docs/engine-copy-kit.md` (§2 wire-the-topology), `docs/release-train.md` calendar, and this chapter plan — all updated in the same PR as the behavior they describe; `make lint` green, no drift. *Gates:* `quality-gate`, `risk-health-check`, `compliance-guardrail`. *Touchpoints:* `docs/`, `docs/plans/`, `docs/decisions/`.
...
- **Phase 15:** Live topology == documented topology (verified via `gh api .../branches`, default-branch, env protection); a `release-on-tag` dispatch blocks on the auto-created open `v0.3.0` milestone without touching `develop`-that-doesn't-exist; a push re-publishes Pages (or equivalent per decision); dependabot opens against the correct branch; ADR 0011 + all docs updated in the same PRs; `make lint` green, no drift.
...
- **Phase 17:** Copy-kit executed end-to-end on a second repo (or the reduced fallback explicitly documented as such); every deviation captured and folded into kit §8; kit copy commands verified byte-correct on the consumer repo; `make lint` green.
