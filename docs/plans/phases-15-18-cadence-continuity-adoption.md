...
Status: **Chapter complete (2026-08-18). Phases 15–17 executed on plan; Phase 16 executed via documented slip decisions (promotion + `v0.3.0` release both early per owner directive — recorded on issues #124/#125, on-time signal reported honestly); Phase 18 close-out in PR #146**. The topology drift is reconciled (ADR 0011, `develop` restored as default at `main` parity, gate-behavior test passed); the chapter operated on the restored two-branch model — train 2 departs **2026-08-31** (cutoff **2026-08-28**) and the copy-kit adoption track completed via the plan's documented in-repo substitution. This chapter made the engine's two remaining promises real: operating at cadence (train 2's window preserved, not inflated) and adoption by a different product.
...
- **P16-T3 — EXECUTED (2026-08-18) as a documented slip decision, not the in-window cadence-compliant release.** Release `v0.3.0` **inside** `[08-31, 09-14)`: close the `v0.3.0` milestone, dispatch `release-on-tag` with `version: 0.3.0`, confirm the GitHub Release publishes with the AI-drafted release-summary appended and the release-cut artifact uploaded. *Gates:* `release-pipeline`, `delivery-telemetry`. *Touchpoints:* milestone (closed), `.github/pdm/workflows/` only if a defect is surfaced (else none).
- **P16-T3 — EXECUTED (2026-08-18, outside train 2's window [08-31, 09-14)).** Per owner directive the window date check was overridden and the release executed as a **documented slip decision** (recorded on issue #125 *before* execution). Milestone `v0.3.0` (#16) closed → dispatched `PDM Release on Tag` `version: 0.3.0` → run [32115048938](https://github.com/frdrpo/learning-pdm-fintech-delivery-engine/actions/runs/32115048938) **success**. `frontend/package.json` bumped 0.2.0 → 0.3.0 on `develop` (`d8922781`); tag **`v0.3.0`** created; GitHub Release published — [Release v0.3.0](https://github.com/frdrpo/learning-pdm-fintech-delivery-engine/releases/tag/v0.3.0) (draft false, prerelease false) with AI-drafted notes appended and `release-notes` + `release-cut-summary` artifacts uploaded. Full record on issue #125. **Honesty note:** early release — on-time truthing in P16-T4 reports it as actually occurred (train 2's window [08-31, 09-14) is not where v0.3.0 landed).
